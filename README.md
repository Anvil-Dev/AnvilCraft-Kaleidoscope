# AnvilCraft: Kaleidoscope

---

Added compatibility for the _AnvilCraft_ mod with the _Kaleidoscope_ series. After installing this mod, you can:
- Use chutes to input items into millstone, stockpot, and pot
- Use chutes to output products from pot
- Squeeze dolls from _Kaleidoscope: Dolls_

This mod also adds the following content:
- Custom dolls:
    - XeKr's doll
    - Gugle's doll
    - _AnvilCraft_ part contributors' dolls
    - _AnvilCraft_ part supporters' dolls

---

为铁砧工艺添加了关于《森罗物语》系列模组的兼容，安装此模组后，你可以：
- 使用溜槽向石磨、汤锅、炒锅输入物品
- 使用溜槽从炒锅输出产物
- 把《森罗物语：玩偶》中的玩偶拍扁

本模组还添加了以下内容：
- 自定义玩偶：
    - XeKr 的玩偶
    - 古镇天的玩偶
    - 《铁砧工艺》部分贡献者的玩偶
    - 《铁砧工艺》部分支持者的玩偶

---

![Use hoppers to input items into millstone](https://cdn.modrinth.com/data/xeCG3WZz/images/0479fdf373667f48b25becbeaff9401fdd125d75.png)
![Use hoppers to input items into stockpot](https://cdn.modrinth.com/data/xeCG3WZz/images/62de9cc7d191937cabd202137df1c0b4ad3bc239.png)
![Use hoppers to input/output items into pot](https://cdn.modrinth.com/data/xeCG3WZz/images/5cebceaf25e41ae1ff2478ee85aad239c7f00455.png)
![Squeeze dolls](https://cdn.modrinth.com/data/xeCG3WZz/images/a4255190672033c71b9a4b18e87e1d0bc148d926.gif)
![dolls](https://cdn.modrinth.com/data/xeCG3WZz/images/ddcdd54a9c78d6ae5f000675fd97f67e35f9170e.png)

## License

* Code unless otherwise stated default to our [LICENSE file(LGPL-3.0)](./LICENSE) here
* Non-Code assets (Located here) go by our [ASSET_LICENSE file(ARR)](./ASSETS_LICENSE) here
